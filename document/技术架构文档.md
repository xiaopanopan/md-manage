...
生产 WebView 使用 CSP：脚本仅允许当前应用；样式允许应用内联样式；连接限制为 Tauri IPC；图片允许受控协议、data/blob 和 HTTP(S)；worker 仅允许同源与 blob。
...
- `selectedEntry` 表示文件树操作对象，`currentFile` 表示编辑器文档；取消文件树选择不会关闭文档。
- 工具栏新建以选择项解析目标：文件夹内部、文件同级、无选择时工作区根目录。
- 新建、重命名和移动完成后展开祖先目录、滚动定位并选中新路径。
- 拖拽仅表示目录移动，只允许文件夹行和根目录空白接收；移动前确认，最近一次移动可撤销。
