...
- Tauri core window/event/menu（`core:default`）；
- `core:window:allow-destroy`：关闭握手唯一需要的窗口写操作；
- dialog open/save；
- opener 的受限 URL 和 reveal；
- 应用自定义 commands。

注意 `core:window:default` **只含只读命令**（`allow-is-*`、尺寸/位置/显示器查询等），不含 `destroy`、`close`、`minimize`、`maximize`、`set-fullscreen`。关闭握手依赖 `destroy`，因此必须显式授予；缺少它会导致 `close-requested` 被 `preventDefault()` 拦住后无法销毁窗口，窗口再也关不掉。

`DesktopAPI.window` 仍声明了 `minimize`、`maximize`、`close`、`setFullscreen`，但没有 UI 调用方，也**未授予**对应权限——按最小权限原则保持未授权状态，接入 UI 时需同步补 capability。
...
- 调用任何窗口写操作（destroy/close/minimize/maximize/fullscreen）前先确认 capability 已授予对应 `core:window:allow-*`，`core:window:default` 只给只读命令。
